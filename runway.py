...
def mainloop():
    ...
    def runway_draw(length_ft,width_ft):    # draw runway (rectangle) for a given length and breadth 
        global image,parking_area_accumulate
    ...
    def parking_rectangle(length,breadth):  # draw parking (rectangle) for a given length and breadth 
        ...
        if parking_gui_input:
            ...
            cross_cen=(int((parking_top_corner[0]+parking_right_corner[0])/2),int((parking_top_corner[1]+parking_right_corner[1])/2))
            cv2.line(image, (cross_cen[0] - 10, cross_cen[1]), (cross_cen[0] + 10, cross_cen[1]), (0,0,255), 2)
            cv2.line(image, (cross_cen[0], cross_cen[1] - 10), (cross_cen[0], cross_cen[1] + 10), (0,0,255), 2)
        cv2.imshow("image", image)
    ...
    def on_mouse_click(event, x, y, flags, param): # functions for when user clicks
        if event == cv2.EVENT_LBUTTONDOWN: 
                ...
                if random.random()<0.85:
                    ...
                    temp_message=''
                    parking_rectangle_points
                    if parking_rectangle_points[0][0]<x<parking_rectangle_points[1][0] and parking_rectangle_points[0][1]<y<parking_rectangle_points[1][1] or parking_gui_input:
                        parking_missile+=1
                        ...
                        if(percent_area>90):
                            color="green"
                            output_text.insert("end", "\n\t\t PARKING DESTROY SUCESSFULLY !\n--------------------------------------------------------------------------\n ")
                            output_text.see("end")
                        else:
                            color="red"
                        canvas1.create_oval(5, 5, 50, 50, fill= color)
                        cv2.imshow('image',image)
                    elif(runway_gui_input):    
                        b=rect_point[1][1]
                        missile.append(1)
    ...
    #scrollbar in GUI window
    canvas = Canvas(frame)
    scrollbar = ttk.Scrollbar(frame, orient="vertical", command=canvas.yview)
    canvas.configure(yscrollcommand=scrollbar.set)
    scrollbar_x = ttk.Scrollbar(frame, orient="horizontal", command=canvas.xview)  # Horizontal scrollbar
    ...
    def main_function():
        ...
        global cross_cen
        global rect_point,missile,parking_missile
        global count
    ...
    runway_button.grid(row=0, column=4, columnspan=7) 
    ...
    file_menu.add_command(label="Refresh", command=do_refresh)
